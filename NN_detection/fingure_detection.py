...
class fingure_top:
    def __init__(self):
        self.lwr = np.array([0, 50, 70], np.uint8)
        self.upr = np.array([100, 230, 230], np.uint8)
    ...
    def pauseT(self, focusF, fingerT, ftop):
        if len(fingerT) < 20:
            fingerT.append(ftop)
        else:
            fingerT.pop(0)
            fingerT.append(ftop)
        self.points(focusF, fingerT)

    def get_fingure_top(self, focusF, are_threshold=3000):
        noise = self.hsvF(focusF)
